...
def _log_lik(COUNT_MATRIX, PT):
    return np.sum(COUNT_MATRIX * PT, dtype=np.float64)
...
def _my_posterior_pre(COUNT_MATRIX, PRE_PT, DIST_SAMPLES):
    ...
    L = np.empty(num_samples, dtype=np.float64)

    for i, pre_pt_element in enumerate(PRE_PT):
        L[i] = _log_lik(COUNT_MATRIX, pre_pt_element)   # log-prob!

    weights = np.ones(num_samples, dtype=np.float64)
    weights[0] = 0.5
    weights[-1] = 0.5

    log_weights = L + np.log(weights, dtype=np.float64)
    log_norm = logsumexp(log_weights) + np.log(DIST_SAMPLES[1] - DIST_SAMPLES[0], dtype=np.float64)
...
def comp_posterior(COUNT_MATRIX_LIST, Q, EQ, DIST_SAMPLES):   
    ...
    PRE_PT = np.array([
        np.log(np.diag(eq64) @ scipy.linalg.expm(q64 * float(DIST_SAMPLE)), dtype=np.float64)
        for DIST_SAMPLE in DIST_SAMPLES], dtype=np.float64)
